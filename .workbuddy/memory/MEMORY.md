...
- **⚠ 已证伪（2026-09-15）**：上轮把 offset 0-7/8-14/15-19 解读为「月初/月中/月末」是**错的**——`offset_grid` 的 offset = 把「每 REBAL=20 交易日调仓」的整张网格平移 N 天，**非日历锚定**（offset=0 调仓日 01-02→01-30→03-06→04-03…月内漂移）。三 anchor 交叉验证：2019 月初高(18.0%)/月中低(10.9%) vs **2021 月初低(5.8%)/月末高(14.3%)**，方向相反 → 纯随机相位噪音，**无月初效应**
- **真实结论：相位敏感性是结构性的**（`scripts/verify_phase_source.py`）：anchor 2019/2020/2021 的 20 相位离散度稳定 3.2~3.7pp、范围 10~12pp → 月度调仓固有特性，非某时点驱动。**单一相位回测 ±5pp 都是噪音**，多相位均值才可信；均值随 anchor 后移下降 14.7%→13.0%→9.9%（市场阶段差异）。稳健性检验须**多 anchor 交叉**，单 anchor 的 offset 模式可能是巧合
- **调仓频率 vs 相位敏感性（2026-09-15）**：REBAL 20→10 离散 3.3→1.9pp（-42%），10→5 不再降（1.9pp）→ **双周是甜点、周度过犹不及**。年化 14.7→13.2→10.8%；零成本归因：双周仅比月度少 0.4pp（=20日因子配10日持有期的期限错配），其余 1.1pp 是换手成本翻倍。**双周 = 用 1.5pp 换回测可信度 ±5pp→±2pp**，生产改动候选，须先配 10 日因子口径 + A/B。REBAL=5 独立相位仅 5 个（offset 0..4），离散估计精度低
